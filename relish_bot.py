...
def log_in(driver, email, password):
  ...
  sign_in_button = driver.find_element(
    By.XPATH, "//input[@name='commit' and @value='Sign in']")
  ...
  try:
    pop_up_button = driver.find_element(
      By.XPATH, "//button[@aria-label='Close modal' and @class='button' and @data-close]")
    pop_up_button.click()
  except NoSuchElementException:
    pass
...
def order(driver, date, meal):
  ...
  restaurant_cards = driver.find_elements(
    By.XPATH, f"//div[@data-role='meal-type-{meal}']/div[contains(@id, 'se-')]")
  if not len(restaurant_cards):
    print("Found no restaurants on " + datestr +
          " for " + meal + ". This is unexpected.")
    return
  ...
  for restaurant_card in restaurant_cards:
    ...
    if element_exists(restaurant_card, "//a"):
      link = restaurant_card.find_element(
        By.TAG_NAME, "a").get_attribute("href")
      restaurants.append(Restaurant(name, link))
  if not len(restaurants):
    print("No available restaurants for " + meal + " on " + datestr +
          ". It may be too late to order or they may be sold out.")
    return

  meal_options = []
  for restaurant in restaurants:
    driver.get(restaurant.link)
    first_menu_cell = driver.find_element(
      By.XPATH, "//div[@class='menu-items']/div[@class='grid-x menu-category-container']")
    if element_exists(first_menu_cell, "//div[@id='previously-ordered']"):
      prev_ordered_items = first_menu_cell.find_elements(
        By.XPATH, "./div[contains(@class, 'menu-item-items')]/div[@class='cell']")
      for item in prev_ordered_items:
        item_name = item.find_element(
          By.XPATH, ".//div[@class='cell-header']").get_attribute("textContent")
        item_price = float(item.find_element(
          By.XPATH, ".//span[@class='menu-item-price']").get_attribute("innerText")[1:])
  ...
  first_menu_cell = driver.find_element(
    By.XPATH, "//div[@class='menu-items']/div[@class='grid-x menu-category-container']")
  chosen_meal_cell = first_menu_cell.find_element(
    By.XPATH, f"./div[contains(@class, 'menu-item-items')]/div[.//div[contains(text(), '{chosen_meal.true_name}')]]")
  chosen_meal_cell.find_element(By.XPATH, ".//a").click()
  click_when_available(
    driver, "//form[@id='new_order_item']//input[@name='commit' and @value='Add to cart']")
  click_when_available(driver, "//a[text()='Proceed to Checkout']")
  click_when_available(
    driver, "//input[@name='commit' and contains(@value, 'Place Order')]")
  print("Success")
...
def main():
  ...
  for date in date_list:
    ...
    for meal in ["lunch", "dinner"]:
      print()
      print(
        f"Ordering {meal} for {date.strftime('%A')} ({date.strftime('%m/%d')})...")
